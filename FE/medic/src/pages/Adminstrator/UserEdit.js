    ...
    const [uEmail, setUEmail] = useState() //emailF
    ...
    const btn_progrm_modify = async (e) => {
        ...
        if (window.confirm("수정하시겠습니까?")) {
            try {
                // userInfo 변수가 초기화된 후에만 호출되도록 변경
                await fetchUserData(); // fetchUserData 함수가 완료될 때까지 대기
                ...
                const response = await axios.put('/admin/manageClient/modify', modifiedUserInfo);
